...
def main():
    ...
    variation_group.add_argument(
        "--listen-interface", type=str, help="Listen on this network interface."
    )
    ...
    if isinstance(args.start_object, rfc1902.ObjectIdentity) or isinstance(
        args.stop_object, rfc1902.ObjectIdentity
    ):
    ...
    if variation_module:
        ...
        try:
            ...
            log.error(
                'Variation module "%s" initialization '
    ...
    if args.listen_interface:
        if not args.quiet:
            log.msg(
                ...
                "mode"
                % (args.listen_interface, "" if args.promiscuous_mode else "non-")
            )
        ...
        try:
            ...
            log.error(
                "Error opening capture file %s for reading: "
    ...
    def handle_snmp_message(d, t, private={}):
        ...
        try:
            rsp_msg, whole_msg = decoder.decode(d["data"], asn1Spec=p_mod.Message())
    ...
    try:
        ...
        for context in contexts:
            ...
            try:
                ...
                log.error(f"writing {filename}: {exc}")
                return 1
            ...
            while True:
                for oid in oids:
                    ...
                    value = values[
                        min(len(values) - 1, bisect.bisect_left(timeline, time_offset))
                    ]
